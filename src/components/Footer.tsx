...
export default function Footer() {
  const menus = [
    ...
    {
      title: "Insights",
      items: ["Testimonials", "Blog", "Webinars", "Career"],
    },
    {
      title: "Contact",
      items: ["Sheffield", "Manchester", "London", "New York", "Contact"],
    },
  ];
  ...
  return (
    <footer className="bg-[#111212] text-white p-4 lg:p-10 mt-3 rounded-3xl mx-3 mb-3 overflow-hidden">
      <div className="max-w-[1600px] mx-auto pt-10">
        {/* Top Grid */}
        <div className="grid grid-cols-2 lg:grid-cols-12 gap-x-6 lg:gap-x-0 gap-y-12 lg:gap-y-0 mb-20">
          {/* Column 1 - Newsletter & Socials */}
          <div className="col-span-2 lg:col-span-6 pr-0 lg:pr-20">
            <h3 className="text-xl lg:text-[30px] font-medium mb-3">Stay Updated with Rise News</h3>
            ...
            <div className="relative mb-3 max-w-md">
              <input
                ...
                className="w-full rounded-full bg-white/10 px-8 py-4 pr-16 focus:outline-none focus:border-white/20 focus:border-2 transition-colors text-lg"
              />
              <button className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-mint rounded-full flex items-center justify-center group hover:bg-white hover:text-grey-900 transition-transform cursor-pointer">
                <i className="fa-solid fa-arrow-right text-grey-900 -rotate-45 group-hover:rotate-45 transition-transform" />
              </button>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {socials.map((social) => (
                <Link
                  key={social.name}
                  href="#"
                  className="flex items-center gap-2 px-2 py-1 bg-white text-grey-900 rounded-full hover:rounded-md transition-transform duration-500 ease-in-out group"
                >
                  <i className={`fa-brands ${social.icon} text-xs`} />
                  <i className="fa-solid fa-arrow-up-right transition-opacity text-xs" />
                </Link>
              ))}
            </div>
          </div>
          ...
          {menus.map((menu, index) => (
            <div
              key={menu.title}
              className={`col-span-1 md:col-span-1 lg:col-span-2 pl-5 border-l border-grey-600`}
            >
              <div className="flex flex-col gap-0.5">
                {menu.items.map((item) => (
                  <Link
                    key={item}
                    href="#"
                    className="text-lg md:text-xl text-white font-medium relative w-fit overflow-hidden group py-0.5"
                  >
        ...
        <div className="w-full lg:pt-10">
          <div className="w-full inline-flex z-100">
            <svg
              className={`w-full h-full object-contain transition-colors text-white`}
        ...
        {/* Bottom Bar */}
        <div className="flex flex-col lg:flex-row gap-1 justify-between pt-10">
          ...
          <div className="text-xs font-medium text-white group">
            <a href="https://syednoor.vercel.app/" target="_blank" rel="noopener noreferrer">Website Made by Syed Noor</a>
            <div className="w-0 h-px bg-white mt-0.5 group-hover:w-full transition-all duration-500" />
          </div>
        </div>
      </div>
    </footer>
