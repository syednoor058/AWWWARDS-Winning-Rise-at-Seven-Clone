...
// Images for the rotating hero image
const heroImages = [
...
const awardsImages = [
  "/awards/global-search-awards.webp",
  "/awards/mask-group.webp",
  "/awards/uk-media-awards-white.webp",
];
...
export default function Hero() {
  ...
  useLayoutEffect(() => {
    ...
    // Scroll animations for hero background
    ...
    tl.to(hero.querySelector(".hero-bg"), { scale: 1.1, duration: 1 }, 0);
    ...
    // Get references
    const categoryWord = titleRef.current?.querySelector(".word-category");
    const leadersWord = titleRef.current?.querySelector(".word-leaders");
    const betweenImage = imageRef.current;

    if (betweenImage) {
      // 1. Capture the final width from classes (w-16, w-24, w-32)
      // Since it's responsive, we'll get the computed width
      const targetWidth = betweenImage.offsetWidth;

      // 2. Set initial state: width 0
      gsap.set(betweenImage, { width: 0, opacity: 0 });

      // 3. Animate width and opacity (this pushes the words naturally in flex)
      gsap.to(betweenImage, {
        width: targetWidth,
        opacity: 1,
        duration: 1.2,
        ease: "power3.inOut",
        delay: 1.2, // Start after letters have mostly landed
      });

      // 4. Also animate the inner image scale for extra polish
      gsap.fromTo(
        betweenImage.querySelector("div"),
        { scale: 1.5 },
        { scale: 1, duration: 1.5, ease: "power3.out", delay: 1.2 }
      );
    }
  ...
  return (
    <section
      ref={heroRef}
      className="relative w-full min-h-screen overflow-hidden rounded-3xl mt-2"
    >
      {/* Background Image - Blurred */}
      <div className="hero-bg absolute inset-0 z-0 transition-transform">
        <div
      ...
      {/* Content Container */}
      <div className="relative z-10 flex flex-col min-h-screen px-4 pt-4 pb-8 lg:px-8 lg:pt-8 lg:pb-12">
        {/* Top spacing */}
        ...
        <div className="text-center mb-auto">
          {/* Award Badge */}
          <div className="flex flex-col items-center justify-center gap-3 mb-4 text-white/90">
            <p className="text-base font-medium max-w-md leading-none">#1 Most Recommended<br />Content Marketing Agency</p>
            <div className="flex items-center gap-2">
              ...
              {awardsImages.map((imgSrc, i) => (
                <div key={imgSrc} className="relative">
                  <img src={imgSrc} alt={`Award-${i}`} className="h-6 w-auto" />
                </div>
              ))}
              <div>
                <svg viewBox="0 0 28 38" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-6 fill-current text-white">
          ...
          {/* Desktop:Main Title with Image Between Words */}
          <div ref={titleRef} className="mb-2 hidden sm:block">
            {/* First Line */}
            <div className="title-line overflow-hidden pb-2">
              <h1 className="text-white text-5xl md:text-7xl lg:text-[120px] font-medium tracking-tight leading-[0.9]">
                {splitText("We Create")}
              </h1>
            </div>
            ...
            <div className="title-line overflow-hidden pb-2">
              <div className="flex items-center justify-center gap-3">
                <span className="word-category inline-block">
                  <span className="text-white text-5xl md:text-7xl lg:text-[120px] font-medium tracking-tight leading-[0.9] inline-block">
                    {splitText("Category")}
                  </span>
                </span>

                {/* Small Square Image Between Words */}
                <div
                  ref={imageRef}
                  className="inline-block w-16 h-16 md:w-24 md:h-24 lg:w-32 lg:h-32 rounded-lg overflow-hidden flex-shrink-0"
                >
                  <div
                ...
                <span className="word-leaders inline-block">
                  <span className="text-white text-5xl md:text-7xl lg:text-[120px] font-medium tracking-tight leading-[0.9] inline-block">
                    {splitText("Leaders")}
                  </span>
                </span>
          ...
          {/* Mobile: Main Title with Image*/}
          <div className="mb-2 block sm:hidden">
            <h1 className="text-white text-[60px] font-medium tracking-tight leading-[0.9] text-center">
              <span>We Create</span>
              <div className="w-full flex items-center justify-center gap-2">
                <span>Category</span>
                <div className="h-14 aspect-square overflow-hidden rounded-lg">
                  <img src={randomImage} alt="hero-image" className="w-full h-full object-cover" />
                </div>
              </div>
              <span>Leaders</span>
            </h1>
          ...
          {/* Subtitle */}
          <p className="text-white/80 text-lg md:text-[30px] mt-4 max-w-2xl mx-auto font-medium">
            on every searchable platform
          </p>
        </div>

        {/* Bottom Paragraphs */}
        <div className="hidden md:flex justify-between gap-8 mt-12 text-white/70">
          <p className="text-[16px] max-w-md font-medium">
            Organic media planners creating, distributing & optimising<br />search-first content for SEO, Social, PR, Ai and LLM search
          </p>
          <p className="text-[16px] max-w-md text-right font-medium">
            4 Global Offices serving<br />UK, USA (New York) & EU
          </p>
        </div>

        {/* Mobile Bottom - Only Right Paragraph */}
        <div className="flex md:hidden justify-center mt-8 text-white/70">
