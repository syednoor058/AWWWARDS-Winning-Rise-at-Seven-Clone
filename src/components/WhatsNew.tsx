...
export default function WhatsNew() {
  ...
  return (
    <section className="py-10 lg:py-20 px-4 lg:px-8 overflow-hidden relative">
      ...
      <div
        ref={cursorRef}
        className="fixed top-0 left-0 w-32 h-32 bg-mint rounded-full z-[100] pointer-events-none hidden lg:flex items-center justify-center opacity-0 scale-0 select-none"
        style={{ left: -25, top: -35 }}
      >
        <i className="fa-solid fa-arrow-up-right text-grey-900 text-[32px]" />
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-8">
            ...
            <h2 className="text-[50px] lg:text-[75px] font-medium tracking-tight leading-none flex items-center">
              ...
              <span className="inline-flex items-center">
                <span className="inline-block w-12 h-12 md:w-16 md:h-16 lg:w-18 lg:h-18 rounded-lg overflow-hidden mx-3">
                  <img src="/whats-new/header.webp" alt="" className="w-full h-full object-cover" />
                </span>
                New
              </span>
            </h2>
          ...
          <div className="hidden md:flex w-full md:w-[1/3] items-center justify-end">
            <PrimaryButton lable="Explore more thoughts" url="/news" />
          </div>
        </div>
        <div className="w-full h-px bg-grey-400 mb-12 hidden md:block" />
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-5">
                <div className="flex items-center gap-2 px-3 py-2 bg-white rounded-full">
                  ...
                  <span className="text-xs font-medium text-grey-600">{item.author.name}</span>
                </div>
                <div className="flex items-center gap-2 px-3 py-2 bg-white rounded-full">
                  <i className="fa-regular fa-clock text-xs text-grey-400" />
                  <span className="text-xs font-medium text-grey-600">{item.time}</span>
                </div>
              </div>

              {/* Title */}
              <h3 className="text-[28px] font-medium leading-none text-grey-900">
                {item.title}
              </h3>
            </div>
          ))}
        ...
        <div className="lg:hidden">
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center gap-2 mb-4">
                    <div className="flex items-center gap-2 px-3 py-1 bg-white rounded-full text-xs">
                      ...
                      <span className="font-medium text-grey-600">{item.author.name}</span>
                    </div>
                    <div className="flex items-center gap-2 px-3 py-1 bg-white rounded-full text-xs">
                      <i className="fa-regular fa-clock text-xs text-grey-400" />
                      <span className="font-medium text-grey-600">{item.time}</span>
                    </div>
                  </div>
                  <h3 className="text-xl font-medium leading-tight text-grey-900">{item.title}</h3>
                </div>
              </SwiperSlide>
            ))}
          </Swiper>
          ...
          <div className="mt-6 mb-10">
            <div className="w-full h-[4px] bg-grey-100 relative overflow-hidden news-tracker" />
          </div>
      ...
      <style jsx global>{`
        .news-tracker.swiper-pagination-progressbar {
          ...
          background: #f3f4f6 !important; /* bg-grey-100 */
        }
